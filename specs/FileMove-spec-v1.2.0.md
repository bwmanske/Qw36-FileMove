...
- When a new or empty JSON file is opened, the `Options` settings must default to `false` for all 3 options.
...
- An `Options` section after `Startup Preview`
...
- When a new or empty JSON file is loaded, default `EnableDirectoryMoves`, `EnableSidecarFiles`, and `HideQueuedSourceFiles` to `false`.
...
- When a directory input is expanded, preserve the directory name and its relative subdirectory structure under each destination directory.
